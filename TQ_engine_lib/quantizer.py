...
class TQEngine:
    ...
    def search_batch(self, queries: torch.Tensor, top_k: int = 100) -> List[Tuple[torch.Tensor, torch.Tensor]]:
        ivf = self.current_ivf_data
        if ivf is None:
    ...
    def search(self, query: torch.Tensor, top_k: int = 100) -> tuple[torch.Tensor, torch.Tensor]:
        ivf = self.current_ivf_data
        if ivf is None:
            raise ValueError("No data indexed. Call index() first.")
        ...
        if isinstance(ivf, IVFData):
            ...
            # ngay cả đối với 1 query duy nhất, thay vì lặp Python.
            if query.dim() == 1:
                query = query.unsqueeze(0)
            
            results = self.search_batch(query, top_k=top_k)
    ...
    def _native_cosine_search_flat(self, query: torch.Tensor, pq: ProdQuantized, top_k: int = 100) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # 2. Tính toán Batch Size theo công thức tỷ số nén
        ...
        for start_idx in range(0, total_vectors, tq_batch_size):
            ...
            # Gọi Rust core để quét Batch này
