...
class ModelHandler:
    ...
    async def handle(self, query, model_name, limit, top_k, language: str = "en"):
        ...
        logger.info(f"  (Adaptive mode: limit & top_k được điều chỉnh theo độ phức tạp query)")
        logger.info("-" * 60)
        ...
        # 3. Select top_k contexts (Adaptive - dynamic top_k)
...
QUESTION: "{query}" """
        ...
        if not answer.startswith("[RAG-Adaptive]"):
            answer = f"[RAG-Adaptive] {answer}"
        ...
        logger.info(f"[RAG-Adaptive] HOÀN THÀNH | Tổng thời gian={total_time:.2f}s")
        logger.info("=" * 60)
